...
class PriceConverter {
  ...
  //       '${price != null ? price.toStringAsFixed(decimalPointCount ?? int.parse(Get.find<SplashController>().configModel.content?.currencyDecimalPoint ?? '2'))
  //       .replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},') : ''}'
  //       '${isRightSide ? getCurrency() : ''}');
  //
  // }
  ...
  static String convertPrice(double? price, {int? decimalPointCount, double? discount, String? discountType, bool isShowLongPrice = false}) {
    // Provide a default value if price is null
    price = price ?? 0.0;
    ...
    return isShowLongPrice == true
        ? '${isRightSide ? '' : getCurrency()}'
        '${price.toStringAsFixed(decimalPointCount ?? int.parse(Get.find<SplashController>().configModel.content?.currencyDecimalPoint ?? '2'))
        .replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (Match m) => '${m[1]},')}'
        '${isRightSide ? getCurrency() : ''}'
        : longToShortPrice('${isRightSide ? '' : getCurrency()}'
        '${price.toStringAsFixed(decimalPointCount ?? int.parse(Get.find<SplashController>().configModel.content?.currencyDecimalPoint ?? '2'))

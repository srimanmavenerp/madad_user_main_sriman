...
class _MenuScreenState extends State<MenuScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final List<MenuModel> menuList = [
      ...
      // MenuModel(
      //   icon: Images.vehicleIcon, // Make sure Images.vehicleIcon exists and points to your vehicle image asset
      //   title: 'Vehicle Model'.tr, // Add translation for 'vehicle_model' if needed
      //   route: RouteHelper.getVehicleModelScreen(), // Implement this route in your RouteHelper
      // ),
      ...
      if((configModel.content?.walletStatus ?? 0) != 0 && isLoggedIn)
        MenuModel(icon: Images.walletMenu, title: 'my_wallet'.tr, route: RouteHelper.getMyWalletScreen()),
      if((configModel.content?.loyaltyPointStatus ?? 0) != 0 && isLoggedIn)
      ...
      //   MenuModel(icon: Images.cancellationPolicy, title: 'cancellation_policy'.tr, route: RouteHelper.getHtmlRoute('cancellation_policy')),
        if((configModel.content?.termsAndConditions ?? "").isNotEmpty)
          MenuModel(icon: Images.termsIcon, title: 'terms_and_conditions'.tr, route: RouteHelper.getHtmlRoute('terms-and-condition')),
      if((configModel.content?.refundPolicy ?? "").isNotEmpty)
        MenuModel(icon: Images.refundPolicy, title: 'refund_policy'.tr, route: RouteHelper.getHtmlRoute('refund_policy')),
      /*if((configModel.content?.cancellationPolicy ?? "").isNotEmpty)
        MenuModel(icon: Images.cancellationPolicy, title: 'cancellation_policy'.tr, route: RouteHelper.getHtmlRoute('cancellation_policy')),
